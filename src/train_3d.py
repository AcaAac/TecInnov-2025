...
def _scale_action_to_velocity(action, v_max: float) -> np.ndarray:
    vec = np.asarray(action, dtype=np.float64).reshape(3)
    return clip_norm(vec * float(v_max), float(v_max))
...
def collect_demonstrations(num_episodes, output_dir, cfg):
    ...
    save_path = os.path.join(output_dir, "expert_3D")
    if torch:
...
def train_bc(states, actions, output_dir, cfg, epochs=50, batch_size=64, val_ratio=0.1, live_plot=False):
    if torch is None:
        raise RuntimeError("PyTorch is required for behavior cloning.")

    print("Training BC Policy for 3D...")

    states = _ensure_numpy(states).astype(np.float32)
    actions = _ensure_numpy(actions).astype(np.float32)
    ...
    agent = PPOAgent(state_dim, action_dim, "CONTINUOUS")
    ...
    for epoch in range(epochs):
        ...
        for bx, by in train_loader:
            ...
            optimizer.zero_grad()
            loss, metric = _bc_batch_metrics(agent, bx, by, "CONTINUOUS", criterion, cfg)
            loss.backward()
        ...
        log_row = {
            ...
            "val_loss": val_loss,
            "train_action_mae": train_metric,
            "val_action_mae": val_metric,
        }
        print(
        ...
        if live_plot:
            _update_bc_live_plot(bc_live_fig, bc_live_axes, pd.DataFrame(training_logs), "CONTINUOUS")

    if best_state_dict is not None:
        agent.model.load_state_dict(best_state_dict)

    save_path = os.path.join(output_dir, "bc_model_3D.pth")
    torch.save(agent.model.state_dict(), save_path)
    print(f"BC Model saved to {save_path} (best epoch: {best_epoch})")

    log_df = pd.DataFrame(training_logs)
    bc_log_path = os.path.join(output_dir, "bc_training_log_3D.csv")
    log_df.to_csv(bc_log_path, index=False)
    print(f"BC training logs saved to {bc_log_path}")

    plot_bc_training_results(log_df, output_dir, "3D")
...
def train_rl(cfg, start_agent=None, timesteps=100000, output_dir="drone_data", steps_per_epoch=2048, live_plot=False):
    if torch is None:
        raise RuntimeError("PyTorch is required for RL fine-tuning.")

    print("Fine-tuning with PPO (3D)...")
    ...
    for epoch in range(epochs):
        ...
        if live_plot:
            _update_rl_live_plot(rl_live_fig, rl_live_axes, pd.DataFrame(training_logs), "3D")

    save_path = os.path.join(output_dir, "rl_model_3D.pth")
    torch.save(agent.model.state_dict(), save_path)
    print(f"RL Model saved to {save_path}")

    log_df = pd.DataFrame(training_logs)
    log_path = os.path.join(output_dir, "training_log_3D.csv")
    log_df.to_csv(log_path, index=False)
    print(f"RL training logs saved to {log_path}")

    plot_rl_training_results(log_df, output_dir, "3D")
...
def evaluate_agents(output_dir, cfg, visualize=False, eval_episodes=50):
    print("\nEvaluating Agents (3D)...")
    env = DroneEnv3D(config=cfg)
    state_dim = env.get_state_dim()
    action_dim = env.get_action_dim()
    ...
    rl_agent = PPOAgent(state_dim, action_dim, "CONTINUOUS")
    rl_path = os.path.join(output_dir, "rl_model_3D.pth")
    ...
    step_csv_path = os.path.join(output_dir, "evaluation_results_3D.csv")
    ...
    episode_csv_path = os.path.join(output_dir, "evaluation_episode_summary_3D.csv")
    episode_df.to_csv(episode_csv_path, index=False)
    print(f"Saved evaluation episode summary to {episode_csv_path}")

    summary_plot_path = os.path.join(output_dir, "evaluation_metrics_3D.png")
    summarize_policy_eval_figure_3d(
        episode_df,
        output_path=summary_plot_path,
        title_prefix="3D Policy Comparison",
    )
...
def main():
    ...
    if args.collect_demos:
        states, actions = collect_demonstrations(args.episodes_demo, output_dir, cfg)
    elif args.train_bc:
        pt_path = os.path.join(output_dir, "expert_3D.pt")
